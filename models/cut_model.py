...
class CUTModel(BaseModel):
    ...
    def compute_D_loss(self):
        ...
        # Real
        self.pred_real = self.netD(self.real_B_op)
    ...
    def compute_G_loss(self):
        ...
        if self.opt.nce_idt and self.opt.lambda_NCE > 0.0:
            ...
            self.loss_NCE_P = self.calculate_NCE_loss(self.real_B1_m, self.fake_B)
    
            self.loss_NCE_Y = self.calculate_NCE_loss(self.real_B, self.idt_B)
            loss_NCE_both = (self.loss_NCE + self.loss_NCE_Y) * 0.5+self.loss_NCE_P
        else:
            loss_NCE_both = self.loss_NCE

        self.loss_G = self.loss_G_GAN
        real_B1_m=self.real_B1_m.detach().cpu().numpy()
        real_A_m=self.real_A_m.detach().cpu().numpy()
        #self.loss_G_L1_B = self.criterionL1(self.real_B_op[np.nonzero(real_B1_m)], self.fake_B[np.nonzero(real_B1_m)]) *10
        ...
        #self.edlos=self.criterionL1(self.snow_seg.float(),self.fake_seg.float())
